...
def insert_csv_record_into_postgres(conn, data):
    cur = conn.cursor()
    insert = """
    ...
    """
    full_date = datetime.strptime(data["date"] + f"/{datetime.now().year}", "%d/%m/%Y").date()
    temp_range = f"{data['min_temp']}-{data['max_temp']}"
    cur.execute(insert, (
        full_date,
        data["weekday"],
        temp_range,
...
def process_message(s3, filename: str, pg_conn):
    ...
    if not data:
        logging.warning("Empty JSON, skipping.")
        return

    df = pd.DataFrame([data])

    append_to_csv(s3, df)
    append_to_parquet(s3, df)
    insert_csv_record_into_postgres(pg_conn, data)
