...
def append_to_parquet(s3, df: pd.DataFrame):
    try:
        obj = s3.get_object(Bucket=PARQUET_BUCKET, Key=PARQUET_FILE)
    ...
    s3.put_object(Bucket=PARQUET_BUCKET, Key=PARQUET_FILE, Body=buffer.getvalue())
    logging.info("Appended data to gold.parquet")

def insert_into_postgres(conn, json_data):
    cur = conn.cursor()
    insert = """
    ...
    """
    for f in json_data["results"]["forecast"]:
        full_date = datetime.strptime(f["full_date"], "%d/%m/%Y").date()
        temp_range = f"{f['min']}-{f['max']}"
        cur.execute(insert, (
            full_date, f["weekday"], temp_range, f["humidity"],
            f["cloudiness"], f["rain"], f["rain_probability"],
            f["wind_speedy"], f["description"], f["condition"]
        ))
    conn.commit()
    cur.close()
...
def process_message(s3, filename: str, pg_conn):
    ...
    forecast_df = pd.DataFrame(raw_json["results"]["forecast"])

    if forecast_df.empty:
        logging.warning("Empty DataFrame, skipping.")
        return

    append_to_csv(s3, forecast_df)
    append_to_parquet(s3, forecast_df)
    insert_into_postgres(pg_conn, raw_json)
